...
describe("OffsetHelper", function () {
  ...
  const dexRouterAddress = "0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506"; // SushiSwap
  const tokenAddresses = addresses[networkName];
  ...
  async function deployOffsetHelperFixture() {
    ...
    const usdc = IERC20__factory.connect(tokenAddresses.USDC, addr2);
    const weth = IERC20__factory.connect(tokenAddresses.WETH, addr2);
    const wmatic = IWETH__factory.connect(tokenAddresses.WMATIC, addr2);
    ...
    await swapper.swap(tokenAddresses.WETH, parseEther("20.0"), {
    ...
    await swapper.swap(tokenAddresses.USDC, parseUSDC("1000"), {
      value: await swapper.calculateNeededETHAmount(
        tokenAddresses.USDC,
        parseUSDC("1000")
      ),
    ...
    await swapper.swap(tokenAddresses.BCT, parseEther("50.0"), {
      value: await swapper.calculateNeededETHAmount(
        tokenAddresses.BCT,
        parseEther("50.0")
      ),
    ...
    await swapper.swap(tokenAddresses.NCT, parseEther("50.0"), {
      value: await swapper.calculateNeededETHAmount(
        tokenAddresses.NCT,
        parseEther("50.0")
      ),
  ...
  describe.only("#autoOffsetExactInToken()", function () {
    ...
    for (const name of TOKEN_POOLS) {
      it(`should retire 1.0 TCO2 using a MATIC swap and ${name.toUpperCase()} redemption`, async function () {
        ...
        const maticCost = await offsetHelper.calculateNeededETHAmount(
          poolToken.name === "BCT" ? tokenAddresses.BCT : tokenAddresses.NCT,
          ONE_ETHER
        );
